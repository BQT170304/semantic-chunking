...
import type { Document, UploadResult, UploadResponse, UploadProgress, WorkerSettings } from "@/types/document";
...
export function DocumentUploadEnhanced({
  ...
  const updateProgress = (filename: string, updates: Partial<UploadProgress>) => {
    setUploadProgress((prev) => prev.map((item) => (item.filename === filename ? { ...item, ...updates } : item)));
  };
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        if (apiResponse.summary.successful_files > 0) {
          const workerInfo =
            apiResponse.summary.workers_used > 1 ? ` using ${apiResponse.summary.workers_used} workers` : "";
    ...
    [onUpload, workerSettings, setIsUploading]
  );
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="secondary" className="flex items-center gap-1">
            ...
            {workerSettings.enableAutoScale ? "Auto" : workerSettings.maxWorkers} workers
          </Badge>
      ...
      {/* Warning when ready to upload */}
      {selectedFiles.length > 0 && !isUploading && (
        <Alert className="mb-4 border-yellow-200 bg-yellow-50">
          <AlertTriangle className="h-4 w-4 text-yellow-600" />
          <AlertDescription className="text-yellow-800">
            Ready to upload {selectedFiles.length} file(s). You cannot add more files or create a new conversation until
            upload is complete.
          </AlertDescription>
        </Alert>
      )}
      ...
      {showSettings && (
        <div className="mb-6 p-4 bg-gray-50 rounded-lg space-y-4">
          <h3 className="text-sm font-medium text-gray-700">Worker Settings</h3>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  setWorkerSettings((prev) => ({
                    ...prev,
                    maxWorkers: Math.max(1, Math.min(8, parseInt(e.target.value) || 1)),
                  }))
                }
                className="w-20"
              />
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {progress.processingTime && (
                    <span className="text-xs text-gray-500">{progress.processingTime.toFixed(1)}s</span>
                  )}
                </div>
              </div>
      ...
      {selectedFiles.length > 0 && !isUploading && (
        <div className="mb-4 p-3 bg-blue-50 rounded-lg">
          <h4 className="text-sm font-medium text-blue-900 mb-2">Selected Files ({selectedFiles.length})</h4>
        ...
        {isUploading ? (
            ...
            <p className="text-xs text-gray-500">
              {uploadProgress.length > 1
                ? `Processing ${uploadProgress.length} files with ${
                    workerSettings.enableAutoScale ? "auto-scaled" : workerSettings.maxWorkers
                  } workers`
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-400">
              Multi-file uploads will use concurrent processing for faster results
            </p>
          </div>
        )}
      </div>
